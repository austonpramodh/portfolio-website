...
const domain = "auston.dev";
const senderEmail = "notifications@auston.dev";
const senderName = "Auston Pramodh Barboza";
const senderReplyToAddress = "austonpramodh@gmail.com";

const themeColor = "#ffffff";

const HostEmailParams = async ({
    name,
    email,
    message,
}: IHostEmailParams): Promise<SendEmailRequest> => {
    ...
    return {
        ...
        Destination: {
            ToAddresses: [`${senderName} <${senderReplyToAddress}>`],
        },
        ReplyToAddresses: [`${name} <${email}>`],
        Message: {
            Body: { Html: { Data: hostEmailTemplateHtml, Charset: "UTF-8" } },
            Subject: {
                Data: "Someone just tried contacting you.",
                Charset: "UTF-8",
            },
        },
    };
